...
offckb deploy --network devnet --target <contract-binary-path> --output F:\ckb\ckb-test-contract/deployment
```
...
```bash
# Deploy to testnet
offckb deploy --network testnet --target <contract-binary-path> --output F:\ckb\ckb-test-contract/deployment
```
